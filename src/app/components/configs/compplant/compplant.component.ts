...
@Component({
  selector: 'app-compplant',
  templateUrl: './compplant.component.html',
  styleUrls: ['./compplant.component.scss']
})
export class CompplantComponent implements OnInit {
  ...
  getFriends(){
    this.httpClient.get<any>('http://122.166.167.113:8085/plant').subscribe(
      response => {
  ...
  onSave() {
    const editURL = 'http://122.166.167.113:8085/plant/' + this.editForm.value.id
  ...
  openDetails(targetModal, Emps) {
   ...
   document.getElementById('id').setAttribute('value', Emps.id);
    ...
    document.getElementById('company').setAttribute('value', Emps.company);
  ...
  onSubmit(f: NgForm) {
    ...
    const url = 'http://122.166.167.113:8085/plant';
  ...
  onDelete() {
    const deleteURL = 'http://122.166.167.113:8085/plant/' + this.deleteId ;
